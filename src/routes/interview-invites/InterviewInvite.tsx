...
export default function AddInterviewInvite({ action }) {
  ...
  const { error, mutateAsync, isPending } = useMutation({
    mutationFn: async (inputValues) => {
      return await client.models.InterviewInvite.create(inputValues, {
        authMode: "userPool",
      });
    },
    onSuccess: () => {
      toast.success("Interview invite added.");
  ...
  function handleAdditionalInformationToggle() {
    setShowAdditionalInformation((prev) => {
      return !prev;
    });
  }
          ...
          <div className="mx-auto grid max-w-[59rem] flex-1 auto-rows-max gap-2 sm:gap-4">
            ...
            In stock
              ...
              <div className="hidden items-center gap-2 md:ml-auto md:flex">
                ...
                <LoadButton size="sm" loading={isPending} type="submit">
                  Save Invite
                </LoadButton>
              </div>
            </div>
                ...
                <Card x-chunk="dashboard-07-chunk-1">
                  <CardHeader>
                    <CardTitle className={`flex justify-between items-center`}>
                      ...
                      {action === "add" && !screenSizes.sm && (
                        <Button
                          variant="ghost"
                          size="sm"
                          className="w-9 p-0"
                          onClick={handleAdditionalInformationToggle}
                          type="button"
                        >
