...
export default function MyInterviewInvitesTable() {
  ...
  const { data: interviewInvites, isLoading: loading } = useQuery({
    queryKey: ["interviewInvites", `my`, `page:${currentPageIndex}`],
    queryFn: async () => {
      try {
        const response = await client.models.InterviewInvite.listInterviewInviteByUserProfileIdAndInviteDateTime(
          {
